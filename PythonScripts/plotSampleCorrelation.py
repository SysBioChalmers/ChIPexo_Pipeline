...
for sample in samples:
    ...
    for i in range(0,len(TFDataRaw)):
        if TFDataRaw[i][0]=='variableStep':
            ...
            TFDataTMP.append([chromTMP,int(TFDataRaw[i][0]),int(TFDataRaw[i][1])])
    #remove part on chrom 12, plasmid-2-micron, mit or unplaced telemore
    filterList=[]
    for i in range(0,len(TFDataTMP)):
        if TFDataTMP[i][0] in filterChromosomes:
            filterList.append(i)
        else:
            for filterRegion in filterRegions:        
                if TFDataTMP[i][0]==filterRegion[0]:
                    if int(TFDataTMP[i][1])>=int(filterRegion[1]) and int(TFDataTMP[i][1])<=int(filterRegion[2]):
                        filterList.append(i)
    for index in sorted(filterList, reverse=True):
        del TFDataTMP[index]
    del index, filterList
    postFilterSum=sum([x[2] for x in TFDataTMP])
    
    #Divide read sum by genome length and scale by 10
    TFbackground=postFilterSum/12147764*10
